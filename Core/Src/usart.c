...
{
  printf("Receive: %c\r\n", uart_rx_buf[0]); // print received data
  if (uart_rx_buf[0] == 'a')                 // if received data is 'a', send 'Hello'
  {
    printf("Hello\r\n");
  }

  HAL_UART_Receive_IT(&huart1, uart_rx_buf, 1); // enable serial port interrupt
}
  // retarget printf
